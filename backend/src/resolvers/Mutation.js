...
const Mutation = {
  ...
  async createPost(parent, { author, title, body, clubName }, { db, pubsub }, info) {
    ...
    let newPost = new db.PostModel({ author: user._id, title, clubName: clubName, body, createTime: Date.now() });
    await newPost.save();
  ...
  async createComment(parent, { commenter, body, postId }, { db, pubsub }, info) {
    if (!commenter || !body || !postId) throw new Error("Missing input for create comment");
    ...
    let newComment = new db.CommentModel({ commenter: user._id, body, createTime: Date.now() });
